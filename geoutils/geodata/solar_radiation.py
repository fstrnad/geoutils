# Taken from https://github.com/google-deepmind/graphcast/blob/main/graphcast/solar_radiation.py
# Only some tiny changes were made to the original code.
#
...
class _OrbitalParameters:
    ...
    theta: np.array
    rotational_phase: np.array
    sin_declination: np.array
    cos_declination: np.array
    eq_of_time_seconds: np.array
    solar_distance_au: np.array
...
def _get_orbital_parameters(j2000_days: np.array) -> _OrbitalParameters:
...
def _get_solar_sin_altitude(
    ...
    sin_latitude: np.array,
    cos_latitude: np.array,
    longitude: np.array,
) -> np.array:
...
def _get_integrated_radiation(
    ...
    return scipy.integrate.trapezoid(fluxes, dx=dx)


# _get_integrated_radiation_jitted = jit(
...
def get_toa_incident_solar_radiation_for_xarray(
    ...
    missing_coords = set(["time", "lat", "lon"]) - \
        set(data_array_like.coords)
    if missing_coords:
        raise ValueError(
