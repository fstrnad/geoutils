...
def apply_cheby_filter(ts, cutoff=4, order=8, fs=1, rp=0.05):
...
def apply_Savitzky_Golay_filter(ts, wl=1, order=3):
    ...
    ts_sav_gol = savgol_filter(ts, window_length=wl, polyorder=order)
    if type(ts) == xr.DataArray:
        ts_sav_gol = xr.DataArray(
            data=ts_sav_gol, dims=ts.dims, coords=ts.coords, name=ts.name
        )

    return ts_sav_gol


def apply_butter_filter(ts, cutoff, order=3, fs=50):
    ...
    if cutoff > 1:
        ...
        if type(ts) == xr.DataArray:
            ts_lp = xr.DataArray(
                data=ts_lp, dims=ts.dims, coords=ts.coords, name=ts.name
            )
    elif cutoff < 1:
...
def apply_med_filter(ts, size=7):
